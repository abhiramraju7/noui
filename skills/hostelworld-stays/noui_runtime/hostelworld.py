"""Public Hostelworld discovery and pricing through Tabby execute/fetch."""
...
DEFAULT_PROFILE = "hostelworld"
PROPERTY_HINTS = ("/hostels/p/", "/pwa/hosteldetails.php")
SEARCH_PATH = "/search"
DESTINATION_PARAM = "search_keywords"
CHECKIN_PARAM = "date_from"
CHECKOUT_PARAM = "date_to"
GUESTS_PARAM = "number_of_guests"
...
async def _fetch_html(profile_slug: str, url: str) -> str:
    ...
    if status == 429:
        raise RuntimeError(
            "Hostelworld returned HTTP 429 inside the Tabby session; retry after the site rate limit clears"
        )
    if status >= 400:
        raise RuntimeError(f"Hostelworld returned HTTP {status} for {url}")
    body = response.get("text", "") if isinstance(response, dict) else ""
    if not body:
        raise RuntimeError("Hostelworld returned an empty page")
    return body
...
def find_stay(search_result: dict[str, Any], stay_id: str) -> dict[str, Any]:
    ...
    if not match:
        raise RuntimeError(f"Hostelworld stay {stay_id!r} was not found")
    return match
