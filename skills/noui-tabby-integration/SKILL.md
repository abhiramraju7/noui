...
| **[reference/provisioning.md](reference/provisioning.md)** | How NoUI provisions **today** (raw `POST /apps` + `POST /admin/profiles`, `login register` vs `tabby setup`), what is stored where, STAGING→ACTIVE, local vs cloud setup, and the **App Template path** (Tabby-built, NoUI-not-built) and how it should be wired. |
...
- **The default `tabby` runtime is agent-token-only.** It ignores `NOUI_TABBY_AUTH_MODE=platform_jwt`, so it can **never** carry `owner_user_id` and can **never** trigger template auto-provisioning. Only `--execution-mode http` honors platform-JWT today.
- **`noui tabby setup --cloud` provisions nothing.** It only verifies the PAT → platform-JWT → Tabby-JWT round-trip and writes env. It creates no App, Profile, or Template.
- **NoUI never sets `execute_enabled`.** Apps are created with the default (`false`). Execute works locally only because the local worker reads `EXECUTE_ENABLED=true` from `tabby/.env.local` and the API uses `LOCAL_WORKER_URL`. In real K8s this would be dead. See [gaps](reference/gaps.md).
