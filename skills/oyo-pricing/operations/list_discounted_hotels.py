#!/usr/bin/env python3
"""List OYO hotels meeting a minimum advertised discount through Tabby."""
...
async def execute(
    ...
    hotels = [
        item for item in result["hotels"] if (item["discount_percentage"] or 0) >= minimum_discount
    ]
    hotels.sort(key=lambda item: item["discount_percentage"] or 0, reverse=True)
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="List discounted OYO hotels.")
