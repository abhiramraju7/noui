"""Public Selina discovery and pricing through Tabby execute/fetch."""
...
BASE_URL = "https://www.socialtel.com"
DEFAULT_PROFILE = "selina"
PROPERTY_HINTS = ("/properties/", ".html")
SEARCH_PATH = "/search"
DESTINATION_PARAM = "search_keywords"
CHECKIN_PARAM = "date_from"
CHECKOUT_PARAM = "date_to"
GUESTS_PARAM = "guests"
...
async def _fetch_html(profile_slug: str, url: str) -> str:
    ...
    if status == 429:
        raise RuntimeError(
            "Selina returned HTTP 429 inside the Tabby session; retry after the site rate limit clears"
        )
    if status >= 400:
        raise RuntimeError(f"Selina returned HTTP {status} for {url}")
    body = response.get("text", "") if isinstance(response, dict) else ""
    if not body:
        raise RuntimeError("Selina returned an empty page")
    return body
...
def _search_url(destination: str, check_in: str, check_out: str, adults: int) -> str:
    # Socialtel currently exposes its migrated Selina inventory as public
    # destination/property pages rather than a server-side search route.
    return f"{BASE_URL}/"
...
def find_stay(search_result: dict[str, Any], stay_id: str) -> dict[str, Any]:
    ...
    if not match:
        raise RuntimeError(f"Selina stay {stay_id!r} was not found")
    return match
