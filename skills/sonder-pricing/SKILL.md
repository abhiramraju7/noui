---
name: sonder-pricing
description: Use this skill when the user wants to inspect live public Sonder rates, compare Sonder stay prices, or find rate differences for a property. Triggers on "Sonder price breakdown", "compare Sonder prices", "cheapest Sonder", or "Sonder discounts". Uses the `sonder` Tabby profile.
---

# Sonder Pricing

Read Sonder's public booking-page rates through Tabby's `POST /execute/fetch`. The skill reports only amounts present in the live response; it does not infer taxes or complete a booking.
...
- Sonder's quote semantics are preserved; do not label a value "per night" unless the source payload explicitly does so.
