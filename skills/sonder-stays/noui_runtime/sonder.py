"""Public Sonder discovery and pricing through Tabby execute/fetch."""
...
BASE_URL = "https://www.sonder.com"
DEFAULT_PROFILE = "sonder"
PROPERTY_HINTS = ("/destinations/", "/units/", "/stays/")
SEARCH_PATH = "/search"
DESTINATION_PARAM = "city"
...
async def _fetch_html(profile_slug: str, url: str) -> str:
    ...
    if status == 429:
        raise RuntimeError(
            "Sonder returned HTTP 429 inside the Tabby session; retry after the site rate limit clears"
        )
    if status >= 400:
        raise RuntimeError(f"Sonder returned HTTP {status} for {url}")
    body = response.get("text", "") if isinstance(response, dict) else ""
    if not body:
        raise RuntimeError("Sonder returned an empty page")
    return body
...
def _search_url(destination: str, check_in: str, check_out: str, adults: int) -> str:
    ...
    slug = urllib.parse.quote(destination.lower().replace(" ", "-"))
    return f"{BASE_URL}/destinations/{slug}/search?{query}"
...
def find_stay(search_result: dict[str, Any], stay_id: str) -> dict[str, Any]:
    ...
    if not match:
        raise RuntimeError(f"Sonder stay {stay_id!r} was not found")
    return match
