---
name: taj-pricing
description: Use this skill when the user wants to inspect live public Taj rates, compare Taj hotel prices, or find rate differences for a property. Triggers on "Taj price breakdown", "compare Taj prices", "cheapest Taj", or "Taj discounts". Uses the `taj-hotels` Tabby profile.
---

# Taj Pricing

Read Taj's public booking-page rates through Tabby's `POST /execute/fetch`. The skill reports only amounts present in the live response; it does not infer taxes or complete a booking.
...
- Taj's quote semantics are preserved; do not label a value "per night" unless the source payload explicitly does so.
