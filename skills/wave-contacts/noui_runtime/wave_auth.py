"""Wave bearer-token resolver (dynamic-header-injection pattern).

The Wave web app authenticates GraphQL calls to ``gql.waveapps.com`` with a
short-lived ``Authorization: Bearer <token>``. The token is sniffed from live
CDP ``Network`` events, cached on disk until shortly before JWT ``exp``, and
re-sniffed on demand.
"""
...
CACHE_PATH = Path("/tmp/noui_wave_bearer.json")
EXP_SKEW = 120
...
async def _sniff(ws_url: str, timeout: float = 25.0) -> str:
    """Reload and capture Authorization from a live waveapps.com / gql request."""
    async with websockets.connect(ws_url, max_size=None) as ws:
        ...
        while loop.time() < deadline:
            ...
            if authz.startswith("Bearer "):
                return authz
    raise RuntimeError(
        "Could not sniff a Wave Authorization token. Ensure Tabby has app.waveapps.com "
        "open and authenticated (run `tabby session ensure --profile wave`)."
    )


async def get_bearer(force: bool = False) -> str:
    """Return a valid ``Bearer <token>`` string, using the disk cache when fresh."""
